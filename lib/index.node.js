...
try {
  ...
  var supportsColor = __webpack_require__(542);
...
var hasFlag = __webpack_require__(166);
...
function supportsColor(haveStream) {
  var _ref = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
      ...
      sniffFlags = _ref$sniffFlags === void 0 ? true : _ref$sniffFlags;
...
function getSupportLevel(stream) {
  ...
  var level = supportsColor(stream, _objectSpread({
...
__webpack_require__.d(__webpack_exports__, {
  ...
  "Vuex": () => (/* reexport */ vuex_esm),
  "actions": () => (/* binding */ actions),
  "createSimplestVuexStore": () => (/* binding */ createSimplestVuexStore),
  "default": () => (/* binding */ src_0),
  "getters": () => (/* binding */ getters),
  "install": () => (/* binding */ src_install),
  "mapActions": () => (/* reexport */ mapActions),
  "mapGetters": () => (/* reexport */ mapGetters)
});

// EXTERNAL MODULE: ./node_modules/debug/src/index.js
var src = __webpack_require__(495);
...
var d = src_default()('simplest-vuex');
var e = src_default()('simplest-vuex:error');
var ds = d.extend('share-space');
var de = d.extend('state');
var da = d.extend('action');
var dm = d.extend('mutation');
var dg = d.extend('getter');
var shareSpace = {
  mutation: {},
  namespace: [],
  action: {},
...
var BuildStoreNS = /*#__PURE__*/function () {
  function BuildStoreNS() {
    ...
    _classCallCheck(this, BuildStoreNS);
  ...
  _createClass(BuildStoreNS, [{
    ...
    value: function speedSetState(localName, params) {
      ...
      de('create', "".concat(localName, " to ").concat(ns, " > ").concat(an));
      storeInstance.state[name] = params; // 设置state

      storeInstance.getters[name] = function (state) {
        return state[name];
      }; // 默认getter
      // 默认action


      storeInstance.actions[an] = function (_ref, payload) {
    ...
    value: function mutation() {
      ...
      dm('create', "".concat(ns, " > ").concat(name));
    ...
    value: function getter() {
      ...
      var ns = this.ns;
      var name = this.getNamespace(n);
      dg('create', "".concat(ns, " > ").concat(name));
      storeInstance.getters[name] = is(cb, Function) ? cb : function (state) {
        return state[name];
      };
      this.shareRegister(shareSpace.getter[ns], name);
    }
    ...
    value: function action(n, cb) {
      ...
      var ns = this.ns;
      var name = this.getNamespace(n);
      da('create', "".concat(ns, " > ").concat(name));

      storeInstance.actions[name] = function (process, params) {
        var actionExtend = _objectSpread(_objectSpread({}, process), {}, {
          ownerState: process.state[ns],
          publish: function publish(nameList) {
            return loop(nameList, function (v, k) {
              return process.commit(_this.getNamespace(k), v);
            });
          },
          ...
          doth: function doth(k, payload) {
            return process.dispatch(_this.getNamespace(k), payload);
          }
        });
    ...
    value: function state() {
      ...
      var ns = this.ns;
      var namespace = shareSpace.namespace; // 初始化注册

      if (!namespace.includes(ns)) {
        namespace.push(ns);
        storeInstance.state[ns] = {};

        storeInstance.getters[ns] = function (state) {
          return state[ns];
        };
      }

      loop(stateList, function (v, n) {
        var storeState = storeInstance.state[ns];
        ...
        de('create', "".concat(ns, " > ").concat(name));
        ...
        _this2.shareRegister(shareSpace.getter[ns], name);

        _this2.shareRegister(shareSpace.mutation[ns], name);

        _this2.shareRegister(shareSpace.state[ns], name);
      });
    }
  }]);
...
var src_d = src_default()('simplest-vuex');
var src_de = src_default()('simplest-vuex:error');
...
var createSimplestVuexStore = function createSimplestVuexStore() {
  return new Store(storeInstance);
};

var src_install = function install(Vue) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  var onEnrollStoreBefore = options.onEnrollStoreBefore;
  Vue.use(vuex_esm);

  if (is(onEnrollStoreBefore, Function)) {
    onEnrollStoreBefore(src_BuildStoreNS, {
      Vuex: vuex_esm,
      Store: Store,
      mapActions: mapActions,
      mapGetters: mapGetters
...
var getters = function getters(ns) {
  ...
  return mapGetters([].concat(_toConsumableArray(toMap(ns, names, shareSpace.getter)), [ns]));
};
...
/* harmony default export */ const src_0 = (src_install);
})();
